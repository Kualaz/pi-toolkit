 ...
 *   config       — load/save <agent-dir>/context-prune/settings.json
...
export default function (pi: ExtensionAPI) {
  ...
  const currentConfig: { value: ContextPruneConfig } = {
    value: { enabled: false, summarizerModel: "default", pruneOn: "every-turn", showSummaryInUI: true },
  };
  ...
  // Pi invalidates the old extension runtime during session replacement/reload.
  // Async flushes can resume after that point, so every flush captures a
  // generation and bails before touching captured `pi` / `ctx` objects if the
  // session changed while the summarizer LLM call was in flight.
  ...
  const isSessionCurrent = (generation: number) =>
    sessionActive && generation === sessionGeneration;

  const isStaleContextError = (err: unknown) =>
    err instanceof Error && err.message.includes("stale after session replacement or reload");
  ...
  const flushPending = async (ctx: ExtensionContext) => {
    ...
    try {
        ...
        pi.sendMessage(
          {
            ...
            display: currentConfig.value.showSummaryInUI,
            details: {
      ...
      if (!isSessionCurrent(generation) || isStaleContextError(err)) return;
      const message = err instanceof Error ? err.message : String(err);
      ctx.ui.notify(`pruner: flush failed: ${message}`, "error");
      ctx.ui.setStatus(STATUS_WIDGET_ID, pruneStatusText(currentConfig.value, statsAccum.getStats()));
    }
  };
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // Load config from <agent-dir>/context-prune/settings.json
  ...
  // Mark this extension instance inactive before Pi invalidates its runtime.
  ...
  pi.on("session_tree", async (_event, ctx) => {
    if (!sessionActive) return;
    // Pending/in-flight flushes belong to the old branch — discard/abandon them.
    sessionGeneration += 1;
    pendingBatches.length = 0;
    indexer.reconstructFromSession(ctx);
    statsAccum.reconstructFromSession(ctx);
