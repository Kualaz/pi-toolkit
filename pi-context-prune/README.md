...
| `/pruner summary-ui` | Show whether future summary messages are visible in the TUI |
| `/pruner summary-ui on\|off` | Show or hide future summary messages in the TUI only; hidden summaries still remain in model context |
...
| `showSummaryInUI` | `true` / `false` | `true` |
...
- `showSummaryInUI: false` hides future summary messages from the TUI only. The custom messages are still written into the session/model context with their toolCallIds, so `context_tree_query` recovery keeps working.
...
flushPending()
  ...
  └─► pi.sendMessage()           inject summary (deliverAs: "steer", display from showSummaryInUI)
