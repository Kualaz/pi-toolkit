...
const SUBCOMMANDS = [
  ...
  { value: "summary-ui", label: "summary-ui — show or hide summary messages in the UI" },
...
function summaryUiLabel(showSummaryInUI: boolean): string {
  return showSummaryInUI ? "visible" : "hidden";
}

function parseSummaryUiArg(value: string): boolean | undefined {
  const normalized = value.toLowerCase();
  if (["on", "true", "yes", "show", "visible", "1"].includes(normalized)) return true;
  if (["off", "false", "no", "hide", "hidden", "0"].includes(normalized)) return false;
  return undefined;
}
...
Usage:
  ...
  /pruner summary-ui                       Show whether summary messages are visible in the UI
  /pruner summary-ui on|off                Show or hide summary messages in the UI only
...
Summary UI visibility:
  - /pruner summary-ui off hides future summary messages from the TUI only.
  - Hidden summaries still remain in model context, so context_tree_query recovery keeps working.
...
export function registerCommands(
  ...
  flushPending: (ctx: ExtensionCommandContext) => Promise<void>,
      ...
      switch (subcommand) {
        ...
        case "settings": {
          ...
          const items: SettingItem[] = [
            ...
            {
              id: "showSummaryInUI",
              label: "Show summaries in UI",
              values: ["true", "false"],
              currentValue: String(config.showSummaryInUI),
              description: "Show context-prune summary messages in the TUI. Set false to keep them in model context while hiding them from the UI.",
            },
            {
              id: "summarizerModel",
          ...
          const onChange = (id: string, newValue: string) => {
            ...
            } else if (id === "showSummaryInUI") {
              newConfig.showSummaryInUI = newValue === "true";
            }
        ...
        case "status": {
          ...
          ctx.ui.notify(
            `pruner status:\n  enabled:    ${cfg.enabled}\n  model:      ${cfg.summarizerModel}\n  trigger:    ${mode}\n  summary UI: ${summaryUiLabel(cfg.showSummaryInUI)}${statsLine}`,
          );
          break;
        }
        ...
        // ── /pruner summary-ui [on|off] ──
        case "summary-ui": {
          const valueArg = subArgs[0];
          if (!valueArg) {
            ctx.ui.notify(`Pruner summary messages in UI: ${summaryUiLabel(currentConfig.value.showSummaryInUI)}.`);
            break;
          }

          const parsed = parseSummaryUiArg(valueArg);
          if (parsed === undefined) {
            ctx.ui.notify(
              `Invalid summary-ui value: "${valueArg}". Use on/off, true/false, show/hide, or visible/hidden.`,
              "warning",
            );
            return;
          }

          currentConfig.value = { ...currentConfig.value, showSummaryInUI: parsed };
          saveConfig(currentConfig.value);
          ctx.ui.notify(
            parsed
              ? "Future pruner summary messages will be shown in the UI."
              : "Future pruner summary messages will be hidden from the UI but kept in model context.",
          );
          break;
        }
