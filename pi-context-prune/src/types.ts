 ...
 *   showSummaryInUI: true = show summary custom messages in the TUI
 *                    false = keep summaries in model context but hide them in the TUI
 *
 ...
 *   display: controlled by config.showSummaryInUI so summaries can stay in
 *   model context without cluttering the TUI.
...
export interface ContextPruneConfig {
  ...
  pruneOn: PruneOn;
  /**
   * Whether summary custom messages should be visible in the Pi TUI.
   * The summary remains in model context either way so context_tree_query works.
   */
  showSummaryInUI: boolean;
}
