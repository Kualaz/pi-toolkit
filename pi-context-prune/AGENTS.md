...
- **`session_start`** — loads config from `<agent-dir>/context-prune/settings.json`, rebuilds the in-memory index and stats accumulator, clears `pendingBatches`, updates the footer status widget, and notifies the user of the loaded state.
...
- **`ContextPruneConfig`** — `{ enabled, summarizerModel, pruneOn, showSummaryInUI }` stored in `<agent-dir>/context-prune/settings.json`.
...
- **`SETTINGS_PATH`** — constant resolving to `<agent-dir>/context-prune/settings.json` (global, project-independent).
...
- **`/pruner settings`** — opens an interactive `SettingsOverlay` (via `ctx.ui.custom()` with `overlay: true`) containing a `SettingsList` with three items:
  ...
  3. **Show summaries in UI** — toggles whether future `context-prune-summary` messages are visible in the TUI while remaining in model context
  ...
  All changes are persisted to `settings.json` on every toggle and the footer widget is updated where relevant.
...
- **`/pruner summary-ui [on|off]`** — gets or sets whether future summary messages are visible in the TUI. Hidden summaries still remain in model context for recovery.
...
| Config in `<agent-dir>/context-prune/settings.json` | Extension owns its own file — no risk of clobbering other Pi settings, and config persists across all projects |
...
| `showSummaryInUI` config | Summary messages stay in model context for recovery, but users can hide them from the TUI to reduce visual noise |
